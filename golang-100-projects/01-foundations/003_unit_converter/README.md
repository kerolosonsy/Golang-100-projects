...
- F1: The program supports exactly one small unit pair per category in the baseline. Temperature: a pair whose conversion involves both a scale and an offset (Celsius and Fahrenheit is the natural baseline). Length: a linear-scale pair. Weight: a linear-scale pair. For each pair, conversions in both directions must be available. Document your chosen pair for each category in your program.
- F2: The program reads a category choice, a source unit, a target unit, and a numeric value from the terminal.
- F3: The program applies the chosen conversion and prints the result.
- F4: An unknown category is rejected with a clear message.
- F5: An unknown unit for a known category is rejected with a clear message.
- F6: A value that cannot be parsed as a number is rejected with a clear message.
- F7: A value that is physically unreasonable for the category (your design choice) is rejected with a clear message.
- F8: Negative inputs follow your chosen policy per category: accepted in temperature (with whatever physical bound you choose), and rejected or accepted in length and weight according to the policy you document.
- F9: After the conversion is reported, the program ends the run; the baseline does not include a repeat loop.
...
- Optional 2: Add a small in-memory history of the last few conversions the program performed, accessible by a terminal command. Keep the same precision policy and the same dispatch style.
