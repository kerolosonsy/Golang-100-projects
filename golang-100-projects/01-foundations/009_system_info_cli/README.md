...
1. F1: The program defines a documented flag for each piece of information it can show. A typical baseline set includes: show the operating system, show the architecture, show the Go version, show a specific environment variable, and a flag to control whether all of the above are printed together.
2. F2: Each flag has a documented default. The defaults compose into a documented overall default behaviour, stated in this README (for example, "by default the program prints operating system, architecture, and Go version, and does not read any environment variable").
3. F3: The program's output for a fixed set of flags and a fixed environment is byte-for-byte reproducible across runs. No time stamp, no random value, no nondeterministic ordering of lines.
4. F4: Environment variable access goes through an explicit, hard-coded allowlist of names. The allowlist lives in the program source. The program reads only the names on the list.
5. F5: The program never calls any function that returns all environment variables, never iterates over the environment, and never prints a list of names that happen to be set.
6. F6: A requested environment variable that is absent is reported with a documented marker (for example, `(unset)`); a present-but-empty variable is reported with a documented marker (for example, `(empty)`). The two cases are distinguishable in the output.
7. F7: The output layout is fixed. Each line is a key, a separator, and a value, terminated by a newline. The same format is used for every line.
8. F8: Unknown flags are rejected by the flag package with a clear error; the program does not silently accept unrecognised flags.
...
Text printed to standard output, one line per piece of information requested. Text-only examples:
...
- **Formal prerequisite:** [Project 008 — Rock Paper Scissors](../../01-foundations/008_rock_paper_scissors/README.md#20-prerequisite-based-documentation-guide).
...
- **API references:** [`flag`](https://pkg.go.dev/flag), [`runtime`](https://pkg.go.dev/runtime).
- **Standards and concept references:** [Go command documentation](https://go.dev/doc/cmd).

### Project-specific learning focus

- **Learn now:** portable system inspection, present-versus-empty environment values, safe output formats, and CLI flag contracts.
