...
1. F1: The game uses a bounded numeric range for the secret and a bounded maximum number of attempts. Both bounds are fixed in the program.
2. F2: The game uses a random source that can be replaced by a test source. The mechanism of replacement is your design.
3. F3: On each attempt, the program reads the user's guess, compares it to the secret, and prints a hint that tells the user how to adjust. If the guess is above the secret, the hint says the secret is lower. If the guess is below the secret, the hint says the secret is higher. The hint is consistent with the comparison and does not reveal the secret.
4. F4: A correct guess ends the game with a message that mentions the secret and the number of attempts used.
5. F5: Exhaustion of attempts without a correct guess ends the game with a message that mentions the secret.
6. F6: Non-numeric input is handled without crashing; the policy on whether it consumes an attempt is yours, must be applied consistently, and must be documented in the code.
7. F7: Numeric input outside the chosen range is handled with a clear message.
8. F8: An end-of-file signal on input is handled without crashing.
...
- **Formal prerequisite:** [Project 003 — Unit Converter](../../01-foundations/003_unit_converter/README.md#20-prerequisite-based-documentation-guide).
...
- **API references:** [`math/rand`](https://pkg.go.dev/math/rand), [`time`](https://pkg.go.dev/time), [`io`](https://pkg.go.dev/io).
- **Standards and concept references:** [A Tour of Go: flow control](https://go.dev/tour/flowcontrol/1).

### Project-specific learning focus

- **Learn now:** loop termination, deterministic random sources, dependency injection, attempt limits, and EOF handling.
